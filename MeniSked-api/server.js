...
const db = knex({
  ...
  connection: {
    connectionString:
      "postgres://vfdbgzlsavrnjk:d3c2d6402dcb3315ff33a87dcf30d4698cac1055c5d33534b6cbfefde65d4041@ec2-54-243-67-199.compute-1.amazonaws.com:5432/d1b7k9s6tas9lh",
...
app.post("/forgot", (req, res) => {
  login.forgotPassword(req, res, db, bcrypt, genPass, transporter);
});
...
app.post("/holiday/r", (req, res) => {
  holiday.addRecurring(req, res, db);
});

//Editing a recurring holiday
app.put("/holiday/r", (req, res) => {
  holiday.editRecurring(req, res, db);
});

//Getting holidays
app.get("/holiday/:type", (req, res) => {
  holiday.getHoliday(req, res, db);
});

//Deleting a holiday
app.delete("/holiday/:type", (req, res) => {
  holiday.deleteHoliday(req, res, db);
});

//Adding a non-recurring holiday
app.post("/holiday/nr", (req, res) => {
  holiday.addNonRecurring(req, res, db);
});

//Scheduling a non-recurring holiday
app.put("/holiday/snr", (req, res) => {
  holiday.skedHoliday(req, res, db);
});

//Deleting a date from a holiday's schedule
app.put("/holiday/esnr", (req, res) => {
  holiday.editSked(req, res, db);
});

//Adding a call type
app.post("/callTypes", (req, res) => {
  calltype.addCall(req, res, db);
});

//Editing a call type
app.put("/callTypes", (req, res) => {
  calltype.editCall(req, res, db);
});

//Deleting a call type
app.delete("/callTypes", (req, res) => {
  calltype.deleteCall(req, res, db);
});

//Getting call types
app.get("/callTypes", (req, res) => {
  calltype.getCall(req, res, db);
});

//Adding a user
app.post("/people", (req, res) => {
  people.addUser(req, res, db, genPass, bcrypt, transporter);
});

//Editing a user
app.put("/people", (req, res) => {
  people.editUser(req, res, db);
});

//Deleting a user
app.delete("/people", (req, res) => {
  people.deleteUser(req, res, db);
});

//Getting all users
app.get("/people", (req, res) => {
  people.getUser(req, res, db);
});

//Editing account information
app.put("/account/:id", (req, res) => {
  account.editAccountInfo(req, res, db);
});

//Editing account's password
app.post("/account/:id", (req, res) => {
  account.editAccountPass(req, res, db, bcrypt);
});

//Getting user's account information
app.get("/account/:id", (req, res) => {
  account.getAccount(req, res, db);
});
...
app.get("/sked/docs", (req, res) => {
  sked.getDocs(req, res, db);
});

//Get entry types
app.get("/sked/entries", (req, res) => {
  sked.getEntries(req, res, db);
});

//Admin assign entry
app.post("/sked/assign", (req, res) => {
  sked.assignEntry(req, res, db);
});

//Admin delete call
app.delete("/sked/assign", (req, res) => {
  sked.deleteSkedCall(req, res, db);
});

//Add entry type
app.post("/entries", (req, res) => {
  entries.addEntry(req, res, db);
});

//Delete entry type
app.delete("/entries", (req, res) => {
  entries.deleteEntry(req, res, db);
});

//Edit entry type
app.put("/entries", (req, res) => {
  entries.editEntry(req, res, db);
});

//Get published months
app.get("/published", (req, res) => {
  published.getMonths(req, res, db);
});

//Update published months
app.put("/published", (req, res) => {
  published.updateMonths(req, res, db);
});
...
app.put("/amessages", (req, res) => {
  messages.messageResponse(req, res, db, transporter);
});

//Employee making a request
app.post("/request", (req, res) => {
  request.addRequest(req, res, db, transporter);
});

//Employee editing a request's dates
app.put("/request", (req, res) => {
  request.editRequest(req, res, db);
});

//Employee cancelling a request
app.put("/drequest", (req, res) => {
  request.cancelRequest(req, res, db);
});

//Admin accepting a request (updates the employee's workSked)
app.put("/arequest", (req, res) => {
  request.acceptRequest(req, res, db);
});

//Get all the departments with MeniSked
app.get("/departments", (req, res) => {
  departments.getDepts(req, res, db);
});
